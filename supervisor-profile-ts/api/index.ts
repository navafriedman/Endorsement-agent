...
function buildComparison(
  ...
  const supervisors = summaries.map(({ profile: p, summary: s }) => ({
    ...
    alignment_overall: p.alignment_scorecard.overall,
...
const PROFILES: Record<string, ResponsivenessProfile> = {
  ...
  luis_alejo: {
    ...
    alignment_scorecard: {
      overall: 'moderate',
      overall_notes: 'District 1 covers Salinas, heavily agricultural. Strong committee fit for ag and waste issues. Former Assemblymember background suggests policy sophistication. Limited direct responsiveness data available.',
      dimensions: {
        voting_alignment: {
          ...
          data_gap: 'Only 3 sample votes — full Legistar extraction needed for comprehensive picture',
          reporter_action: 'Run full Legistar vote extraction for 2021-present',
        },
        committee_relevance: {
          label: 'Committee Relevance',
          score: 'strong',
          evidence: ['Salinas Valley Solid Waste Authority Chair — directly serves district waste management', 'LAFCO Commissioner — shapes service delivery boundaries', 'Air Resources District — relevant to agricultural air quality'],
          reporter_action: 'Review committee meeting minutes for Alejo\'s participation level',
        },
        accessibility: {
          label: 'Accessibility',
          score: 'insufficient_data',
          evidence: ['Official email and phone publicly listed', 'Former Assemblymember — experienced with constituent services'],
          data_gap: 'No town hall records, response time data, or social media engagement metrics',
          reporter_action: 'File CPRA for constituent correspondence volume and response times',
        },
        donor_independence: {
          label: 'Donor Independence',
          score: 'insufficient_data',
          evidence: ['Agricultural industry support reported but dollar amounts not extracted'],
          data_gap: 'Need Cal-Access filing analysis to map donors to votes',
          reporter_action: 'Pull Cal-Access filings and cross-reference top donors with vote record',
        },
        issue_responsiveness: {
          label: 'Issue Responsiveness',
          score: 'moderate',
          evidence: ['Farmworker housing vote aligns with district priority', 'Groundwater sustainability engagement aligns with agricultural district needs'],
          data_gap: 'No Change.org petition response tracking, no public comment analysis',
          reporter_action: 'Search Change.org for District 1 petitions; analyze board meeting public comment periods',
        },
      },
    },
  ...
  glenn_church: {
    ...
    alignment_scorecard: {
      ...
      overall_notes: 'First-term supervisor with limited public track record. District 2 includes Monterey Peninsula — Fort Ord and water are top constituent concerns. Too few data points for reliable assessment.',
      dimensions: {
        voting_alignment: {
          label: 'Voting Alignment',
          score: 'insufficient_data',
          evidence: ['Only 2 sample votes available, both unanimous board actions'],
          data_gap: 'Need full Legistar extraction — any dissenting votes would be most informative',
          reporter_action: 'Run full Legistar vote extraction; flag any non-unanimous votes',
        },
        committee_relevance: {
          label: 'Committee Relevance',
          score: 'moderate',
          evidence: ['TAMC seat relevant to Peninsula transportation', 'Water Management District liaison matches top constituent concern', 'Fort Ord successor body matches cleanup advocacy'],
        },
        accessibility: {
          label: 'Accessibility',
          score: 'insufficient_data',
          evidence: ['Official contact info published'],
          data_gap: 'No town hall records, limited social media, no response time data',
          reporter_action: 'File CPRA for constituent correspondence; survey District 2 community orgs for accessibility feedback',
        },
        donor_independence: {
          label: 'Donor Independence',
          score: 'insufficient_data',
          evidence: ['Minimal campaign finance reporting found for first-time candidate'],
        ...
        issue_responsiveness: {
          label: 'Issue Responsiveness',
          score: 'insufficient_data',
          evidence: ['Fort Ord cleanup is top district issue — board addressed timeline in 2024', 'Desal petition (2,400 signatures) represents significant constituent concern — supervisor position unclear'],
          data_gap: 'No petition response data, no public comment analysis',
          reporter_action: 'Contact Change.org petition organizers; review board meeting videos for Church responses during Fort Ord and water agenda items',
        },
      },
    },
    meeting_minutes_availability: {
  ...
  chris_lopez: {
    ...
    alignment_scorecard: {
      overall: 'moderate',
      overall_notes: 'Most data-rich profile due to longest tenure on current board after Adams. Clear tensions between committee/vote record and campaign finance sources make Lopez the strongest story angle for examining alignment.',
      dimensions: {
        voting_alignment: {
          label: 'Voting Alignment',
          score: 'moderate',
          evidence: [
            'Voted for farmworker housing (aligns with South County ag district)',
            'Voted for cannabis licensing expansion (serves on Cannabis Committee)',
            'Voted for hiring freeze (board-wide, but constituents reported service delays)',
          ],
          data_gap: 'Only 4 sample votes; full Legistar extraction needed',
          reporter_action: 'Extract all votes from 2019-present; identify any dissents or 4-1 splits where Lopez was the swing',
        },
        committee_relevance: {
          label: 'Committee Relevance',
          score: 'strong',
          evidence: [
            '12 committee seats — most of any current supervisor',
            'Budget Committee Vice Chair — direct fiscal influence',
            'Cannabis, Energy/Environment, Water committees match South County priorities',
            'California Coastal Commission — statewide appointment, significant development power',
            'Rural County Reps (First Vice Chair) — statewide rural advocacy',
          ],
          reporter_action: 'Story angle: Does holding 12 seats spread attention too thin, or does it maximize influence? Compare attendance across committees.',
        },
        accessibility: {
          label: 'Accessibility',
          score: 'insufficient_data',
          evidence: [
            'Official email/phone published',
            'Active social media (@LopezForSup, @lopezforsupervisor)',
          ],
          data_gap: 'No response time data, town hall records, or public comment engagement analysis',
          reporter_action: 'File parallel CPRA requests to all 5 offices for constituent email volume and response times — compare Lopez to colleagues',
        },
        donor_independence: {
          label: 'Donor Independence',
          score: 'weak',
          evidence: [
            'Known fossil fuel contributions (Chevron, CIPA) while serving on Energy/Environment committee',
            'Coastal Commission appointment drew scrutiny over development-friendly stance',
            'Environmental groups questioned Coastal Commission appointment',
          ],
          data_gap: 'Specific dollar amounts not extracted from Cal-Access',
          reporter_action: 'Pull Cal-Access filings for 2018 and 2022 races; map donor industries to committee assignments and votes. Key question: do fossil fuel donors get favorable outcomes?',
        },
        issue_responsiveness: {
          label: 'Issue Responsiveness',
          score: 'moderate',
          evidence: [
            'Farmworker housing vote matches #2 constituent issue',
            'Change.org oil industry petition (8,500 signatures) — supervisor position unclear despite Energy committee role',
            'Desal petition (2,400 signatures) — position unclear despite Water committee role',
            'Groundwater sustainability engagement (LandWatch tracking)',
          ],
          data_gap: 'Petition response data not available; public comment engagement unmeasured',
          reporter_action: 'Cross-reference the 6 identified constituent issues with Lopez\'s full vote record and public statements. Interview LandWatch for their assessment.',
        },
      },
    },
    meeting_minutes_availability: {
  ...
  wendy_root_askew: {
    ...
    alignment_scorecard: {
      overall: 'moderate',
      overall_notes: 'Committee assignments closely match stated district priorities (housing, childcare, community energy). District 4 (Seaside/Marina) faces acute housing and Fort Ord issues. Alignment appears intentional but limited vote data prevents confirmation.',
      dimensions: {
        voting_alignment: {
          label: 'Voting Alignment',
          score: 'moderate',
          evidence: ['Voted for farmworker housing density bonus (aligns with housing advocacy)', 'Voted for hiring freeze (mixed — may impact social services her constituents rely on)'],
          data_gap: 'Only 3 sample votes available',
          reporter_action: 'Extract full Legistar votes; identify housing and social services votes specifically',
        },
        committee_relevance: {
          label: 'Committee Relevance',
          score: 'strong',
          evidence: [
            'Housing Authority Commissioner — District 4 has acute housing affordability issues',
            'First 5 Monterey County — childcare advocacy matches stated priorities',
            'Community Action Partnership — anti-poverty focus relevant to Seaside/Marina demographics',
            'Central Coast Community Energy — clean energy access for lower-income residents',
          ],
          reporter_action: 'Compare committee attendance rates and outcomes for housing-related items',
        },
        accessibility: {
          label: 'Accessibility',
          score: 'insufficient_data',
          evidence: ['Official contact published', 'Active social media presence noted'],
          data_gap: 'No response time or town hall data',
          reporter_action: 'CPRA request; also contact Seaside/Marina community organizations for informal accessibility assessment',
        },
        donor_independence: {
          label: 'Donor Independence',
          score: 'insufficient_data',
          evidence: ['Campaign finance data in Cal-Access but not yet analyzed'],
          reporter_action: 'Pull Cal-Access filings; look for housing developer contributions vs. housing affordability votes',
        },
        issue_responsiveness: {
          label: 'Issue Responsiveness',
          score: 'moderate',
          evidence: [
            'Fort Ord cleanup directly impacts District 4 — board addressed timeline',
            'Housing advocacy matches top constituent concern',
            'Childcare funding advocacy matches reported community needs',
          ],
          data_gap: 'No petition tracking for District 4 specifically',
          reporter_action: 'Search Change.org for Seaside, Marina, and Fort Ord petitions; cross-reference with Root Askew positions',
        },
      },
    },
    meeting_minutes_availability: {
  ...
  mary_adams: {
    ...
    alignment_scorecard: {
      overall: 'moderate',
      overall_notes: 'Longest-serving current supervisor with most institutional knowledge. Budget Committee Chair gives outsized fiscal influence. Cannabis vote dissent demonstrates willingness to break with board majority. Big Sur advocacy is visible and directly constituent-serving. Carmel Valley development concerns less clearly addressed.',
      dimensions: {
        voting_alignment: {
          label: 'Voting Alignment',
          score: 'moderate',
          evidence: [
            'Cannabis licensing dissent — only "No" vote in sample, potentially reflecting District 5 constituency preferences',
            'Budget approval and hiring freeze votes are board-wide consensus items',
          ],
          data_gap: 'Longest Legistar history (2017+) but only 3 sample votes extracted',
          reporter_action: 'Adams has most vote history available — full extraction would show evolution over 3 terms. Look for dissent patterns.',
        },
        committee_relevance: {
          label: 'Committee Relevance',
          score: 'strong',
          evidence: [
            'Budget Committee Chair — most powerful fiscal role on the board',
            'TAMC Board Chair — transportation directly impacts Big Sur and Carmel Valley commuters',
            'Big Sur Multi-Agency Advisory Council — direct district advocacy',
            'Marine Sanctuary Advisory Council — coastal protection relevant to district',
          ],
          reporter_action: 'Interview Budget Committee participants about Adams\'s agenda-setting influence',
        },
        accessibility: {
          label: 'Accessibility',
          score: 'insufficient_data',
          evidence: ['Official contact published', 'District 5 geography (Big Sur to Carmel Valley) makes in-person access challenging'],
          data_gap: 'No town hall records — but Big Sur remoteness makes this especially important',
          reporter_action: 'CPRA request; also contact Big Sur community orgs (e.g., Big Sur Land Trust) about supervisor accessibility',
        },
        donor_independence: {
          label: 'Donor Independence',
          score: 'insufficient_data',
          evidence: ['Three election cycles of Cal-Access data available (2016, 2020, 2022) — most history of current board'],
          data_gap: 'Dollar amounts and donor industries not analyzed',
          reporter_action: 'Three-cycle donor analysis could reveal patterns. Cross-reference with Carmel Valley development decisions.',
        },
        issue_responsiveness: {
          label: 'Issue Responsiveness',
          score: 'moderate',
          evidence: [
            'Big Sur Highway 1 advocacy — vocal Caltrans lobbying matches top district concern',
            'Cannabis vote dissent may align with Carmel Valley/Big Sur constituency preferences',
            'Oil industry petition (8,500 signatures) — position unclear',
            'Carmel Valley traffic concerns (CVA) — supervisor action unclear',
          ],
          data_gap: 'No petition response tracking',
          reporter_action: 'Interview Carmel Valley Association about Adams responsiveness to traffic/development concerns',
        },
      },
    },
    meeting_minutes_availability: {
